...
void access_array(int x) {
  ...
  if((x / throttle[0] / throttle[page_size] / throttle[8192] / throttle[3 * page_size]) <
...
int main(int argc, const char **argv) {
  ...
  // page aligned
  mem = (char *)(((size_t)_mem & ~0xfff) + page_size * 2);
  // initialize memory
  ...
  for (int zz = 0; zz < 100000*(sizeof(DATA_SECRET)-1); zz++) {
    ...
    if (j >= sizeof(DATA) - 1) {
      ...
      for (i = 0; i < 256; i++) {
        ...
        if (MEASURE(mem + mix_i * page_size)) {
/*
