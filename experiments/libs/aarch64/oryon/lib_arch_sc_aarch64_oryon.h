...
measure_arch_sc(void *victim) {
    ...
    void **call_a = (void**)(exec_mapping + ((random() % exec_mapping_size) & (~127)));
    *call_a = (void*)&return_to;

    uint32_t* exec_page;
    do {
        // Randomization reduces prefetcher-driven I-cache preloads.
        exec_page = (uint32_t*)(exec_mapping + ((random() % exec_mapping_size) & (~127)));
    ...
    // Full barrier here. This brings down false hits a lot. 20 seems to be the sweet spot
    mfence();
    nospec();
    for(int i = 0; i < 20; i++) mfence();
